    ...
    global const half* b)
{
    ...
    event_t completion_b = 0;
    for (int i = 0; i < 16; i++)
    {
        completion_b = async_work_group_copy(&b_tile[next][i][0], (global int16 *)(b + k_stride * i), k_subgroup_count, completion_b);
    }
    ...
    for (int k = 0; k < k_iterations; k++)
    {
        ...
        if (k + 1 < k_iterations)
        {
            for (int i = 0; i < 16; i++)
            {
                completion_b = async_work_group_copy(&b_tile[next][i][0], (global int16 *)(b + k_stride * i + (k+1) * 32 * k_subgroup_count), k_subgroup_count, completion_b);
            }
        }
    ...
    if (sid == 0)
    {
        coalesced_store_16(c, c_reg, tid);
    }
}