    ...
    global const half* b)
{
    ...
    for (int i = 0; i < 16; i++)
    {
        completion_b = async_work_group_copy(&b_tile[next][i][0], (global int16 *)(b + (j * 16 + i) * k_stride), k_subgroup_count, completion_b);
    }
    ...
    for (int k = 0; k < k_iterations; k++)
    {
        ...
        if (k + 1 < k_iterations)
        {
            for (int i = 0; i < 16; i++)
            {
                completion_b = async_work_group_copy(&b_tile[next][i][0], (global int16 *)(b + (j * 16 + i) * k_stride + (k+1) * 32 * k_subgroup_count), k_subgroup_count, completion_b);
            }
        }
